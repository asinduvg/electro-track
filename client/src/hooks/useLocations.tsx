...
function useLocations() {
    ...
    return await dbOperation<Location>(
      ...
      (location) =>
        setLocations((prevLocations) =>
          prevLocations.map((prevLocation) => (prevLocation.id === id ? location : prevLocation))
        ),
      setError,
      ERR_LOCATION_UPDATE
    );
  ...
  const totalValueInLocation = (locationId: string, stocks: Stock[], items: Item[]) => {
    return stocks
      .filter((stock) => stock.location_id === locationId)
      .flatMap((stock) =>
        items.filter((item) => item.id === stock.item_id).map((item) => ({ ...stock, item }))
      )
      .map((stock) => stock.quantity * Number(stock.item.unit_cost))
      .reduce((sum, quantity) => sum + quantity, 0);
  };

  const totalInventoryValue = (stocks: Stock[], items: Item[]) =>
    locations
