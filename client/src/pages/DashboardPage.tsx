...
const DashboardPage: React.FC = () => {
    ...
    const lowStockItems = items.filter(
        (item) => item.status === ItemStatus.LOW_STOCK,
    );

    // Get recent transactions (last 5)
    const recentTransactions = [...transactions]
        .sort((a, b) => {
            const dateA = a.performed_at
                ? new Date(a.performed_at).getTime()
                : 0;
            const dateB = b.performed_at
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <Card className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white border-0 shadow-xl shadow-slate-900/20">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-slate-300 font-medium text-sm uppercase tracking-wide">
                                    Total Components
                                </p>
                ...
                <Card className="bg-gradient-to-br from-amber-500 via-orange-500 to-red-500 text-white border-0 shadow-xl shadow-orange-500/20">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-orange-100 font-medium text-sm uppercase tracking-wide">
                                    Critical Stock
                                </p>
                                <p className="text-3xl font-bold mt-2">
                ...
                <Card className="bg-gradient-to-br from-emerald-600 via-green-600 to-teal-600 text-white border-0 shadow-xl shadow-green-500/20">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-green-100 font-medium text-sm uppercase tracking-wide">
                                    Portfolio Value
                                </p>
                                <p className="text-3xl font-bold mt-2">
                                    $
                ...
                <Card className="bg-gradient-to-br from-violet-600 via-purple-600 to-indigo-700 text-white border-0 shadow-xl shadow-purple-500/20">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-purple-100 font-medium text-sm uppercase tracking-wide">
                                    Recent Activity
                                </p>
                                <p className="text-3xl font-bold mt-2">
                        ...
                        <div className="mt-4 flex items-center text-sm">
                            <Link
                                ...
                                className="flex items-center hover:text-purple-100 transition-colors"
                            >
                                <ArrowUpRight className="h-4 w-4 mr-2" />
                        ...
                        {lowStockItems.length > 0 ? (
                            <div className="space-y-3">
                                {lowStockItems.map((item) => (
                                    <div
                                        key={item.id}
                                        className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200 hover:border-orange-200 transition-colors"
                                    >
                            ...
                            <div className="text-center py-12">
                                ...
                                <p className="text-slate-500">
                                    All components are above their minimum
                                    thresholds
                                </p>
                            </div>
                        )}
                        ...
                        <div className="space-y-4">
                            {getItemDetailsForTransactions(
                                ...
                                return (
                                    <div
                                        key={txnWithItem.id}
                                        className="flex items-center space-x-4 p-3 rounded-lg border border-slate-100 hover:border-slate-200 transition-colors"
                                    >
                                        <div
                                            className={`w-10 h-10 ${config.bg} rounded-lg flex items-center justify-center text-sm`}
                                        >
                                            {config.icon}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-medium text-slate-900">
                                                {txnWithItem.item
                                                    ? txnWithItem.item.name
                                                    : "Unknown Component"}
                                            </p>
                                            <p className="text-xs text-slate-500">
                                                {config.label} •{" "}
                                                {txnWithItem.quantity > 0
                                                    ? "+"
                                                    : ""}
                                        ...
                                        <div className="text-xs text-slate-400">
                                            {txnWithItem.performed_at &&
                                            typeof txnWithItem.performed_at ===
                                                "string"
                                                ? new Date(
                                                      txnWithItem.performed_at,
                                                  ).toLocaleDateString()
                            ...
                            >
                                <div className="flex flex-col items-start">
                                    ...
                                    <p className="text-sm text-slate-600 leading-relaxed">
                                        Register new electronic components
                                    </p>
                                </div>
