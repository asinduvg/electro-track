...
const DISPOSAL_REASONS = ['Damaged', 'Expired', 'Obsolete', 'Defective', 'Lost', 'Other'] as const;
...
const DisposeItemsPage: React.FC = () => {
  ...
  const handleReasonChange = (itemId: string, reason: string) => {
    setSelectedItems(
      selectedItems.map((item) => (item.id === itemId ? { ...item, reason } : item))
    );
  };

  const handleNotesChange = (itemId: string, notes: string) => {
    setSelectedItems(selectedItems.map((item) => (item.id === itemId ? { ...item, notes } : item)));
  };

  const handleLocationChange = (itemId: string, locationId: string) => {
    setSelectedItems(
      selectedItems.map((item) => (item.id === itemId ? { ...item, locationId } : item))
    );
  };
    ...
    const invalidItems = selectedItems.filter((selectedItem) => {
      ...
      return (
        ...
        selectedItem.quantity >
          getQtyInLocation(selectedItem.id, selectedItem.locationId, stocks) ||
        !selectedItem.reason
      );
    });
                          ...
                          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div className="col-span-2">
                              ...
                              >
                                ...
                                {availableLocations(selectedItem.id, locations, stocks).map(
                                  (location) => (
                      ...
                      filteredItems.map((item) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            <Badge
                              variant={
                                getTotalQuantity(item.id, stocks) === 0
                                  ? 'danger'
