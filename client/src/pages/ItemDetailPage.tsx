  ...
  if (error || !item) {
    return (
      <div className="py-12 text-center">
        <h2 className="mb-4 text-2xl font-bold text-gray-900">{error || 'Item not found'}</h2>
        <Link to="/inventory">
    ...
    return [
      {
        ...
        label: `${getStatusLabel(automaticStatus)} (Automatic)`,
        disabled: true,
      },
      { value: 'discontinued', label: 'Discontinued', disabled: false },
    ];
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center space-x-4">
            <Link to="/inventory">
              <Button
                variant="outline"
                className="flex items-center border-slate-200 bg-white hover:bg-slate-50"
              >
                <ArrowLeft className="mr-2 h-4 w-4" />
                Back to Inventory
              </Button>
            ...
            <div>
              ...
              <p className="font-medium text-slate-500">SKU: {item.sku}</p>
            </div>
          </div>
          {currentUser &&
                ...
                {isEditing ? (
                  <>
                    ...
                    <Button
                      ...
                      className="flex items-center bg-[#FF385C] px-6 py-3 font-medium text-white shadow-lg transition-colors hover:bg-[#E31C5F] hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
                    >
                  ...
                  <div className="space-y-4">
                    <div className="relative aspect-square overflow-hidden rounded-2xl border border-slate-200 bg-slate-100">
                      {editedItem.image_url || item.image_url ? (
                        ...
                        <div className="flex h-full w-full items-center justify-center border-2 border-dashed border-slate-300">
                          <div className="text-center">
                            <Package className="mx-auto mb-4 h-16 w-16 text-slate-400" />
                            <p className="font-medium text-slate-500">No image available</p>
                          </div>
                        </div>
                      )}

                      {isEditing && (
                        <div className="absolute inset-0 flex cursor-pointer items-center justify-center bg-black bg-opacity-50 opacity-0 transition-opacity hover:opacity-100">
                          <label className="cursor-pointer rounded-lg bg-white px-4 py-2 font-medium text-slate-900 transition-colors hover:bg-gray-100">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="space-y-4">
                        {/* Name Field */}
                        <div>
                          <label className="mb-1 flex items-center text-sm font-medium text-slate-600">
                            Name
                          ...
                          {isEditing ? (
                            <div className="space-y-2">
                              <select
                                ...
                                className="w-full rounded-md border border-slate-200 px-3 py-2 focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]"
                              >
                              ...
                              <p className="text-xs text-slate-500">
                                Stock status (In Stock, Low Stock, Out of Stock) updates
                                automatically based on inventory levels. Only "Discontinued" can be
                                set manually.
                              </p>
                            </div>
                          ) : (
                        ...
                        <div>
                          ...
                          {isEditing ? (
                            ...
                            <p className="font-medium text-slate-900">{item.minimum_stock || 0}</p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          {isEditing ? (
                            <textarea
                              ...
                              className="min-h-[80px] w-full rounded-md border border-slate-200 px-3 py-2 focus:border-[#FF385C] focus:ring-2 focus:ring-[#FF385C]"
              ...
              <CardContent>
                {itemStocks.length > 0 ? (
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <div>
                              <p className="font-semibold text-slate-900">
                                {location?.unit || 'Unknown Location'}
                              </p>
                              <p className="text-sm text-slate-500">
                                {location?.building || location?.room || 'Storage location'}
                              </p>
                            </div>
                          </div>
                  ...
                  <div className="py-8 text-center">
                    <Warehouse className="mx-auto mb-4 h-12 w-12 text-slate-400" />
          ...
          <div className="space-y-6">
            ...
            <Card className="border-0 bg-white shadow-lg">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="border-t border-slate-200 pt-4">
                  ...
                  <div className="mt-2 flex items-center justify-between">
                    ...
                    <span className="font-medium">
                      ${(totalQuantity * Number(item.unit_cost)).toFixed(2)}
                    </span>
                  </div>
                  <div className="mt-2 flex items-center justify-between">
                ...
                <div className="border-t border-slate-200 pt-4">
                  <div className="text-center">{getStatusBadge(item.status)}</div>
                </div>
              </CardContent>
            </Card>
          </div>
