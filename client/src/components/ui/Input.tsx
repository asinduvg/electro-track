...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    { className, label, helperText, error, fullWidth = false, leftAddon, rightAddon, ...props },
    ref
  ) => {
    return (
      <div className={cn(fullWidth && 'w-full')}>
        ...
        <div className={cn('relative rounded-md', leftAddon ? 'flex' : '')}>
          ...
          <input
            ref={ref}
            className={cn(
              'block w-full rounded-md border-2 px-3 py-2 text-gray-900 placeholder-gray-400 shadow-sm',
