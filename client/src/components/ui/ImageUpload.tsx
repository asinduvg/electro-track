...
export const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {previewUrl && (
        <div className="relative inline-block">
          <img
            src={previewUrl}
            alt="Item preview"
            className="rounded-lg border border-gray-300 object-cover shadow-sm"
      ...
      {!previewUrl && (
        <div
          className={`relative rounded-lg border-2 border-dashed p-6 text-center transition-colors ${isDragOver ? 'border-blue-400 bg-blue-50' : 'border-gray-300'} ${disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-gray-400'} `}
          onDrop={handleDrop}
      ...
      {/* Helper text */}
      {selectedFile && (
        <p className="text-xs text-gray-500">Image will be uploaded when you save the item</p>
      )}
    </div>
  );
};
